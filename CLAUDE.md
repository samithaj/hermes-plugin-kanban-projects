# CLAUDE.md — hermes-plugin-kanban-projects

Context for any Claude Code session working in this repo. Read before editing.

## What this is

A **standalone, git-installable Hermes dashboard plugin** (`kanban-projects`). It renders the Hermes
Kanban board grouped by **project path** (Claude Code session cwd), joined with a human-owned
...
half is separate Claude Code hooks, see "Related work").
...
- **Two boards, joined by canonical project key:**
  - `claude-code-work` (the session mirror — written every minute by `~/.hermes/scripts/sync_claude_kanban.py`; **never** write to it from here).
...
- Vault: `/Users/sam/Documents/obsidian_vault_pro/` (repo `samithaj/notes`). If a session ever creates
  notes, follow that vault's `CLAUDE.md`: edit `wiki/`, never `raw/`, append `log.md`. Note-join is
...
- Live test: `cp -r dashboard ~/.hermes/plugins/kanban-projects/` then `GET http://127.0.0.1:9120/api/dashboard/plugins/rescan` (or restart the dashboard). View at
  `http://127.0.0.1:9120/kanban-projects?board=claude-code-work`.
...
- `~/.hermes/plans/2026-06-23_223519-warp-hermes-claude-agentview-orchestration.md` — companion plan:
  the Warp bridge (`:9777` `/open` `/compose` `/send`), the session registry, and the `project_key`
  the triage strip's links depend on.
- `~/.claude/agent-alert-plan.md` — the **audio** half of sound→navigate: per-session Claude Code
  hooks (`Notification: permission_prompt|idle_prompt` start; `UserPromptSubmit`/`PostToolUse`/`Stop`
  stop) that play a repeating sound when an agent needs you. Those hooks live in `~/.claude/hooks/`
